...
test('keywords match boundaries, plurals, and hyphenated phrases', () => {
  assert.deepEqual(keywordHits('Wind turbines, fuel cells and iron-air batteries.', ['wind', 'turbine', 'fuel cell', 'iron air', 'batteries']), ['wind', 'turbine', 'fuel cell', 'iron air', 'batteries'])
  assert.deepEqual(keywordHits('The window is open. A microgrid is not the word grid.', ['wind']), [])
})
test('source text strips tags and decodes entities without rendering markup', () => {
  assert.equal(plainText('<jats:p>Solar &amp; storage &#x26; grids</jats:p>'), 'Solar & storage & grids')
...
test('OSTI rejects missing, short, and overlong queries before fetching', async () => {
  for (const q of ['', 'a', 'x'.repeat(161)]) {
    const response = await onRequestGet({request: new Request(`https://example.test/api/osti?q=${q}`)})
...
test('OSTI adapter fixes upstream host, bounds results, streams JSON, and handles failure', async () => {
  ...
  try {
    ...
    const response = await onRequestGet({request: new Request('https://example.test/api/osti?q=solar%20%26%20wind')})
    assert.equal(response.status, 200)
    assert.equal((await response.json())[0].osti_id, '123')
    globalThis.fetch = async () => new Response('', {status:503})
    assert.equal((await onRequestGet({request: new Request('https://example.test/api/osti?q=solar')})).status, 502)
    globalThis.fetch = async () => { throw new Error('offline') }
    assert.equal((await onRequestGet({request: new Request('https://example.test/api/osti?q=solar')})).status, 504)
