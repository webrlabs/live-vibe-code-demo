...
export default function Research({ onAnalyze }: { onAnalyze: (text: string) => void }) {
  ...
  async function measureAnnual() {
    ...
    setAnnualLoading(true); setAnnualError('')
    const finalYear = new Date().getFullYear() - 1
    try {
      const counts = await Promise.all([finalYear - 2, finalYear - 1, finalYear].map(async year => {
        const url = new URL('https://api.crossref.org/works')
        url.searchParams.set('query', searched); url.searchParams.set('rows', '0')
        url.searchParams.set('filter', `type:journal-article,from-pub-date:${year}-01-01,until-pub-date:${year}-12-31`)
        const response = await fetch(url, { signal: AbortSignal.timeout(20000) })
        if (!response.ok) throw new Error('Annual counts are unavailable. Please retry shortly.')
        const data = await response.json()
        const count: unknown = data.message?.['total-results']
        if (typeof count !== 'number' || !Number.isFinite(count)) throw new Error('The source did not return usable counts.')
        return { year, count }
      }))
      setAnnual(counts)
    } catch (e) { setAnnualError(e instanceof Error && e.name !== 'TimeoutError' ? e.message : 'Annual counts timed out. Please retry.') }
    finally { setAnnualLoading(false) }
  }
  async function search() {
    if (query.trim().length < 2 || loading || annualLoading) return
    setLoading(true); setError(''); setAnnual([]); setAnnualError(''); setPapers([]); setSearched(''); setFilter('All')
    try {
      let results: Paper[]
      if (source === 'Crossref') {
        const url = new URL('https://api.crossref.org/works')
  ...
  return <section className="research"><div className="panel research-search"><div><p className="eyebrow">LIVE PUBLIC RESEARCH</p><h2>Follow the signal to its source.</h2><p className="muted">Search publications, extract technology signals, and explore potential applications.</p></div><form onSubmit={e => { e.preventDefault(); void search() }}><label className="research-query">Research topic<input value={query} onChange={e => setQuery(e.target.value)} placeholder="e.g. perovskite solar cells" minLength={2} maxLength={160} required disabled={loading || annualLoading}/></label><label>Public source<select value={source} onChange={e => setSource(e.target.value)} disabled={loading || annualLoading}><option>Crossref</option><option>OSTI</option></select></label><button className="button dark" disabled={loading || annualLoading || query.trim().length < 2}>{loading ? 'Searching…' : 'Discover research ↗'}</button></form><div className="research-suggestions"><span>Try a direction</span>{['energy storage', 'green hydrogen', 'perovskite solar', 'smart grid'].map(q => <button key={q} disabled={loading || annualLoading} onClick={() => setQuery(q)}>{q} ↗</button>)}</div></div>
    ...
    <div className="panel annual-panel"><div className="section-heading"><div><p className="eyebrow">RESEARCH ACTIVITY OVER TIME</p><h3>Go beyond this sample</h3><p>Compare complete calendar years using Crossref journal-article search counts.</p></div><button className="button secondary" disabled={annualLoading} onClick={() => void measureAnnual()}>{annualLoading ? 'Measuring…' : annual.length ? 'Refresh annual counts' : 'Measure annual activity ↗'}</button></div>{annualError && <p className="notice error" role="alert">{annualError}</p>}{annual.length > 0 && <><div className="annual-counts">{annual.map(a => <div key={a.year}><span>{a.year}</span><strong>{a.count.toLocaleString()}</strong><div className="signal-track"><span style={{width: `${a.count / Math.max(...annual.map(v => v.count), 1) * 100}%`}}/></div><small>matching journal articles</small></div>)}</div><p className="chart-note">Crossref keyword search for “{searched}”. Counts reflect metadata coverage and search relevance; they do not establish technology adoption or market growth. Broad queries can include unrelated articles. Complete years only; current year excluded.</p></>}</div>
