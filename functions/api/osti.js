...
export async function onRequestGet(context) {
  const query =
    new URL(context.request.url).searchParams.get("q")?.trim() || "";
  if (query.length < 2 || query.length > 160)
    return Response.json(
      { error: "Use a search between 2 and 160 characters." },
      { status: 400 },
    );
  ...
  try {
    ...
    return Response.json(
      { error: "OSTI did not respond in time. Try Crossref or retry later." },
      { status: 504 },
    );
  }
}
